...
//自动化
 // 获取 URL 参数中的 prediction 值
    const urlParams = new URLSearchParams(window.location.search);
    const prediction = urlParams.get('prediction');

    // 如果有 prediction 参数，自动选择对应的科室
    if (prediction) {
        const departmentIndex = departments.indexOf(prediction); // 获取 prediction 对应的科室索引
        if (departmentIndex !== -1) {
            departmentSelect.value = departmentIndex; // 设置为对应的科室选项
        }
    }

// 排队按钮点击事件处理
queueButton.addEventListener('click', function () {
    const selectedDepartment = queueDepartmentSelect.value;
    const patientName = queuePatientNameInput.value;
...
function updateQueueTable() {
    ...
    Object.entries(queues).forEach(([department, info]) => {
        if (info.count > 0) {
            ...
            estimatedTimeCell.textContent = info.count * 60; // 假设每个患者预计需要1分钟
...
function updateDetailedQueueTable() {
    ...
    Object.entries(queues).forEach(([department, info]) => {
        info.patients.forEach((patientName, index) => {
            ...
            const queueTime = Math.floor((Date.now() - info.joinTimeList[index]) / 1000); // 计算排队时间
            ...
            // 预计排队时间（每个患者预计需要 60 秒）
            const estimatedTimeCell = document.createElement('td');
            const estimatedTime = info.count * 60; // 假设每个患者需要1分钟
    ...
    Object.entries(queues).forEach(([department, info]) => {
        info.patients.forEach((patientName, index) => {
            if (patientName === searchName) {
                ...
                const estimatedTime = info.count * 60;
                const endTime = new Date(info.joinTimeList[index] + estimatedTime * 1000);
...
function checkQueueTime() {
    Object.entries(queues).forEach(([department, info]) => {
        if (info.count > 0) {
            ...
            if (queueTime >= info.count * 60) { // 假设每个患者需要1分钟
                // 发出叫号
...
function updateCalledTable() {
    ...
    calledRecords.forEach(record => {
        ...
        reservedCell.textContent = record.reserved ? '是' : '否';
