...
class PlotFrame(ttk.Frame):
    def __init__(self, parent):
        ...
        # 初始化缩放比例变量 (默认1.0表示不缩放)
        self.scale_var = tk.DoubleVar(value=1.0)

        # 创建图形和极坐标轴
        self.fig = Figure(figsize=(6, 4), facecolor='none')  # 透明背景
        ...
        # 初始化散点图（红色，大小100，置于顶层）
        self.scatter = self.ax.scatter([], [], c='red', s=100, zorder=10)

        # 创建Canvas控件
        self._setup_canvas()

        # 添加比例缩放控件
        self._add_scale_control()

    def _setup_polar_axes(self):
        """配置极坐标轴参数"""
        self.ax.set_theta_zero_location('N')  # 0度指向北方
        self.ax.set_theta_direction(-1)  # 顺时针方向
        self.ax.set_thetamin(-90)  # 最小角度-90度
        self.ax.set_thetamax(90)  # 最大角度90度
        self.ax.set_rlim(0, 500)  # 半径范围0-500
        self.ax.set_rticks([100, 200, 300, 400, 500])  # 半径刻度
        self.ax.set_rlabel_position(0)  # 半径标签位置
        self.ax.set_thetagrids([-90, -45, 0, 45, 90])  # 角度网格线
    ...
    def _load_background_map(self):
        """加载并显示背景地图"""
        try:
            # 尝试加载地图文件（支持png/jpg等格式）
            bg_img = plt.imread('map.png')
            ...
            print("警告：未找到地图文件'map.png'，将不显示背景")
        except Exception as e:
            print(f"加载地图时出错: {str(e)}")
    ...
    def _add_scale_control(self):
        """添加比例缩放滑块控件"""
        scale_frame = ttk.Frame(self)
        scale_frame.pack(fill=tk.X, pady=5)

        ttk.Label(scale_frame, text='地图比例:').pack(side=tk.LEFT, padx=5)
        ...
        self.scale_slider.pack(fill=tk.X, expand=True, padx=5)

        # 显示当前比例值
        ttk.Label(scale_frame, textvariable=self.scale_var).pack(side=tk.RIGHT, padx=5)
    ...
    def _on_scale_change(self, event=None):
        ...
        # 更新半径范围（保持比例）
        self.ax.set_rlim(0, 500 * scale_factor)
    ...
    def update_plot(self, distance, azimuth):
        ...
        # 应用当前比例缩放
        scaled_distance = distance * self.scale_var.get()

        # 更新散点位置
        self.scatter.set_offsets(np.c_[azimuth_rad, scaled_distance])
